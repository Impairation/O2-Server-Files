...
<%
'检查用户物品
dim total_gem,total_mcash,total_seq,total_AItem
total_gem	= 0
total_mcash	= 0
total_seq	= 0
total_AItem	= 0

dim sql_gift,sql_gift2
dim g_gem,g_mcash,g_flag


sql_gift = "SELECT i_seq,memberID,pid,gName,i_gem,i_price,i_point,gFlag,regDate FROM TB_GIFT_IMSI_CART Where memberID='"& gameid &"'"  
Set giftRS	= Conn.Execute(sql_gift)
If Not(giftRS.BOF or giftRS.EOF) Then	'判断购物袋是否为空 
while Not(giftRS.BOF or giftRS.EOF)
	if trim(giftRS("gFlag"))="I" or trim(giftRS("gFlag"))="S" then
		If Trim(giftRS("gFlag")) = "I" Then total_AItem = total_AItem + 1

		total_gem	= total_gem + giftRS("i_gem")
		total_mcash	= total_mcash + giftRS("i_price")
		total_seq	= total_seq +1
	else	'物品为套装
		sql_gift2 = "SELECT ItemType, ItemID, ItemGem, ItemMCash FROM T_O2JAM_SETITEM_DATA WHERE SetItemID = " & int(giftRS("pid"))
		set giftRS2 = Conn1.Execute(sql_gift2)
		if Not(giftRS2.BOF or giftRS2.EOF) Then	'判断套装是否为空 
		while Not(giftRS2.BOF or giftRS2.EOF)
			if giftRS2("ItemType")=1 then total_AItem = total_AItem + 1

			total_gem	= total_gem + giftRS2("ItemGem")
			total_mcash	= total_mcash + giftRS2("ItemMCash")
			total_seq	= total_seq +1
			giftRS2.movenext()
		wend
		end if
...
<table width="710" border="0" cellspacing="0" cellpadding="0"><form  method = "post"  name="order_info" action = "gift_step_03.asp" onSubmit="return sendit()">
  <tr> 
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<%Elseif total_gem > Gem or total_mcash > MCash then%>
			<a href="gift_step_01.asp?gameid=<%=gameid%>" onfocus='this.blur()'><img src="/images/payment/payment_but_back_off.gif" height="35" border="0" onMouseOver="this.src='/images/payment/payment_but_back_on.gif'" onMouseOut="this.src='/images/payment/payment_but_back_off.gif'"></a>
...
<%
	...
	If Not(objDBrs.BOF or objDBrs.EOF) Then
		...
		%>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr> 
                <td align="right" class="txtWhite">总数量(<span class="txtYellowB"><strong><%=ea%></strong></span>种) 
                  : <span class="txtYellowB"><%=FormatNumber(sum,0)%> Ｍ×120% / <%=FormatNumber(sumGem,0)%> Ｇ　( <font color=white>Ｍ币加收20%赠送税费</font> )</span><!-- (储备金 : <%=FormatNumber(sumPoint,0)%>点) --></td>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="txtDarkblue">
				...
				<% Else %>
					...
					<% If strCashType = 1 Then '酒官鸥 酒捞袍老 版快%>
						<td width="25%"><span style="display:none"><input checked type=radio name="chkCashType" value="PC"<%If MCash < sum Then%> disabled<%End If%>><b>MCASH</b>(<%=FormatNumber(MCash,0)%>)</span></td>
						<td width="25%"><span style="display:none"><input type=radio name="chkCashType" value="OC"<%If O2Cash < sum Then%> disabled<%End If%>><b>O2Cash</b>(<%=FormatNumber(O2Cash,0)%>)</span></td>
						<td width="25%"><span style="display:none"><input type=radio name="chkCashType" value="AC"<%If ItemCash < sum Then%> disabled<%End If%>><b>ItemCash</b>(<%=FormatNumber(ItemCash,0)%>)</span></td>
						<td width="25%"><span style="display:none"><input type=radio name="chkCashType" value="MC" disabled><b>MusicCash</b>(<%=FormatNumber(MusicCash,0)%>)</span></td>
					<% ElseIf strCashType = 2 Then '孤流 酒捞袍老 版快%>
						<td width="25%"><span style="display:none"><input checked type=radio name="chkCashType" value="PC"<%If MCash < sum Then%> disabled<%End If%>><b>MCASH</b>(<%=FormatNumber(MCash,0)%>)</span></td>
						<td width="25%"><span style="display:none"><input type=radio name="chkCashType" value="OC"<%If O2Cash < sum Then%> disabled<%End If%>><b>O2Cash</b>(<%=FormatNumber(O2Cash,0)%>)</span></td>
						<td width="25%"><span style="display:none"><input type=radio name="chkCashType" value="AC" disabled><b>ItemCash</b>(<%=FormatNumber(ItemCash,0)%>)</span></td>
						<td width="25%"><span style="display:none"><input type=radio name="chkCashType" value="MC"<%If MusicCash < sum Then%> disabled<%End If%>><b>MusicCash</b>(<%=FormatNumber(MusicCash,0)%>)</span></td>
