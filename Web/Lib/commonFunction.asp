<%
	...
	'''〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓
	'''〓〓〓 문자열을 자를때 한글과 영문, 숫자를 다르게 자르는 함수 〓〓〓〓〓
	
	'''〓 제목등을 출력할때 한글과 영문의 글자폭차이 때문에 글자를 자를때
	'''〓 무척 복잡해집니다. 같은 글자수로 자르면 한글은 밀려나고
	'''〓 한글에 맞추면 영문은 턱없이 부족하게 출력되고
	'''〓 그래서 한글(영문, 숫자를 제외한 모든 문자)은 2자로 계산하여
	'''〓 글자 자르는 위치를 반환하여 원하는 위치에서 자를 수 있게 합니다.
	'''〓 예)
	'''〓 "베토벤 - 엘리제를 위하여"
	'''〓 "Live 베토벤 Wire -=full version=-"
	'''〓 베토벤 - 엘리제를...
	'''〓 Live 베토벤 Wire -...
	'''〓
	'''〓 위치만 반환해줍니다.
