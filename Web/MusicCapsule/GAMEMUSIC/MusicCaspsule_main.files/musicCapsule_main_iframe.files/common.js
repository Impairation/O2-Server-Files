			...
			// 입력된 내용이 숫자 그리고 하이픈(Hyphen) 주로 전화번호 입력용입니다.
			function CheckNumeric_Hyphen(objInput)
			{
				strInput = objInput.value;
				intLength = strInput.length;
				...
				for(i = 0; i < intLength; i ++)
				{				
					if(!(strInput.substr(i, 1).match(reNumeric)) && !(strInput.substr(i, 1).match(reHyphen)))
					{
						return false;
					}
				}				
			...
			function iframeResize(ifrm) 
			{
			    ...
			    try {
			        ...
			        var oBody = eval(ifrm + ".document.body");	
			        var intFrameHeight;        
			        
			        intFrameHeight = oBody.scrollHeight + (oBody.offsetHeight - oBody.clientHeight);
			        if(intFrameHeight == 0)
			        {
						intFrameHeight = 300;
			        }
			        oFrame.style.height = intFrameHeight
			...
			function getLoginCleck(getURL)
			{
				...
				{
					cookieSubArr	= cookieArr[i].split("=");
					cookieSubArr[0]	= cookieSubArr[0].replace(" ", "");
					if (cookieSubArr[0] == cookieName)
					{
						varValue = cookieSubArr[1];
						break;
					}
				}
				if		(varValue.length < 1) {varValue = -1;}
				else	{varValue = unescape(varValue);}
														
				if (varValue == -1)
				{
					document.LoginIframe.play();
				}
				else
				{
					varValue = varValue.replace("_O2", "");
								
					getURL += "&gameid=" + varValue;
					ifrmResizeSub.location.href	= getURL;
				}
			}
